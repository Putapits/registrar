...
$q = $conn->prepare("SELECT * FROM student_contacts WHERE student_id = ?");
...
<head>
    ...
    <title>Guardian & Emergency Contact</title>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; font-family:'Outfit',sans-serif; }
        ...
        .profile-card { background: white; padding: 3rem; border-radius: var(--radius-lg); box-shadow: 0 10px 40px rgba(0,0,0,0.05); }
        .info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 2rem; margin-top: 2rem; }
        .info-item { margin-bottom: 1.5rem; }
        .info-label { font-size: 0.85rem; color: var(--text-muted); font-weight: 600; margin-bottom: 0.5rem; }
        .info-value { font-size: 1.1rem; font-weight: 500; color: var(--text-main); background: #f8fafc; padding: 0.8rem 1.2rem; border-radius: 12px; border: 1px solid #e2e8f0; }
        .alert-info { background: #fee2e2; color: #ef4444; padding: 1rem; border-radius: 12px; margin-top: 2rem; font-size: 0.9rem; }
    </style>
</head>
<body>
    <?php include 'sidebar.php'; ?>
    <main class="main-content">
        <h1 style="margin-bottom: 2rem;">Guardian & Emergency Contact</h1>
        <div class="profile-card">
            <div class="info-grid">
                <div class="info-item">
                    <div class="info-label">Guardian Name</div>
                    <div class="info-value"><?php echo $data['guardian_name'] ?? 'Not Set'; ?></div>
                </div>
                <div class="info-item">
                    <div class="info-label">Relationship</div>
                    <div class="info-value"><?php echo $data['relationship'] ?? 'Not Set'; ?></div>
                </div>
                <div class="info-item">
                    <div class="info-label">Contact Number</div>
                    <div class="info-value"><?php echo $data['contact_number'] ?? 'Not Set'; ?></div>
                </div>
                <div class="info-item">
                    <div class="info-label">Email</div>
                    <div class="info-value"><?php echo $data['email'] ?? 'Not Set'; ?></div>
                </div>
                <div class="info-item" style="grid-column: span 2;">
                    <div class="info-label">Guardian Address</div>
                    <div class="info-value"><?php echo $data['address'] ?? 'Not Set'; ?></div>
                </div>
            </div>
            <div class="alert-info">
                This information is <strong>VIEW ONLY</strong>. Contact the office for updates.
            </div>
        </div>
    </main>
</body>
